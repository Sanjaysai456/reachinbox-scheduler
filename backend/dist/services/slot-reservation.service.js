...
exports.reserveEmailSlot = void 0;
...
`;
const reserveEmailSlot = async ({ senderId, desiredAt, delayMs, hourlyLimit, }) => {
    const [scheduledAt, hourBucket] = (await redis_1.redis.eval(RESERVATION_SCRIPT, 2, `reachinbox:sender:${senderId}:available-at`, `reachinbox:sender:${senderId}:hour:`, desiredAt.toString(), delayMs.toString(), hourlyLimit.toString()));
    return {
        scheduledAt: new Date(scheduledAt),
        hourBucket: new Date(hourBucket),
    };
};
exports.reserveEmailSlot = reserveEmailSlot;
